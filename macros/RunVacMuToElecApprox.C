...
#include<TString>
    ...
    //savename
    TString fprob="comp_vac_MuToElec";
    ...
    */
    ...
    for (Int_t i=1; i < npoints+1; i++){
        ...
        ymodel3flav_mat[i-1]=model3flav_vac_kopp_MuToElec(x);
    }
    ...
    for (Int_t i=1; i < npoints+1; i++){
        ...
        ymodel3flav_mat_2nd_kopp[i-1]=model3flav_vac_kopp_2nd_MuToElec(x);
    ...
    for (Int_t i=1; i < npoints+1; i++){
        ...
        ymodel3flav_mat_1stalpha[i-1]=model3flav_vac_1stalpha_MuToElec(x);
    }
    ...
    for (Int_t i=1; i < npoints+1; i++){
        ...
        ymodel3flav_mat_1stth13[i-1]=model3flav_vac_1stsin13_MuToElec(x);
    }
    ...
    TGraph *fmodel3flav_vac_apx_nocp=0;
    Double_t xmodel3flav_vac_apx_nocp[npoints], ymodel3flav_vac_apx_nocp[npoints];
    
    for (Int_t i=1; i < npoints+1; i++){
        x[0]=i/(npoints/10.) + epsilon;
        xmodel3flav_vac_apx_nocp[i-1]=i/(npoints/10.) + epsilon;
        ymodel3flav_vac_apx_nocp[i-1]=model3flav_vac_apx_nocp_MuToElec(x);
    }
    
    fmodel3flav_vac_apx_nocp = new TGraph(npoints,xmodel3flav_vac_apx_nocp,ymodel3flav_vac_apx_nocp);
    ...
    fmodel3flav_vac_apx_nocp->SetLineColor(kGray+2);
    fmodel3flav_vac_apx_nocp->SetLineWidth(2);
    ...
    fmodel3flav_vac_apx_nocp->Draw("same");
    leg->AddEntry(fmodel3flav_mat_1stth13,"3flav vac 1st th13","l");
    leg->AddEntry(fmodel3flav_mat_1stalpha,"3flav vac 1st alpha","l");
    leg->AddEntry(fmodel3flav_mat_2nd_kopp,"3flav vac 2nd only","l");
    leg->AddEntry(fmodel3flav_mat,"3flav vac ana","l");
    leg->AddEntry(fmodel3flav_vac_apx_nocp,"3flav nocp appx","l");
    leg->Draw();
